...
def save_document_content(content: Dict, document_id: str, version: str) -> str:
    """
    コンテンツをファイルに保存し、ストレージキーを返す
    ローカル開発環境用のシンプルな実装
    """
    # ドキュメント保存ディレクトリ
    storage_dir = os.path.join("storage", "documents", str(document_id))

    # ディレクトリが存在しない場合は作成
    os.makedirs(storage_dir, exist_ok=True)

    # ファイルパス
    file_path = os.path.join(storage_dir, f"{version}.json")
...
def seed_document_data_sync(session: Session) -> None:
    ...
    for doc_data in sample_documents:
        ...
        # ストレージディレクトリ作成
        os.makedirs(os.path.join("storage", "documents", str(doc.id)), exist_ok=True)

        # リビジョンを作成
        for rev_data in doc_data["revisions"]:
            # コンテンツを保存
            storage_key = save_document_content(rev_data["content"], doc.id, rev_data["version"])
